...
export function ResidentHeader({ onMenuOpen: _onMenuOpen }: { onMenuOpen?: () => void }) {
  ...
  const alerts = useQuery(
    ...
    profile?.societyId && blockId ? { societyId: profile.societyId, blockId } : "skip"
  );
  ...
  return (
    <header className="h-14 flex items-center justify-between px-4 shrink-0 header-glass sticky top-0 z-20">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => router.push("/resident/notices")}
        ...
        <div className="relative" ref={userRef}>
          ...
          {userOpen && (
            <div
              role="menu"
              className="absolute right-0 top-full mt-1 bg-card border border-white/[0.08] rounded-lg shadow-lg py-1 z-50 min-w-[160px]"
            >
